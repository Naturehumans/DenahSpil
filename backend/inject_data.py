...
async def inject_data():
    async with AsyncSessionLocal() as db:
        ...
        if not floor:
            floor = Floor(name="Lantai 1 Demo", level=1)
            db.add(floor)
            await db.commit()
        ...
        if not category:
            category = EquipmentCategory(name="APAR", color="#cf2821", icon_type="fire-extinguisher")
            db.add(category)
            await db.commit()
        ...
        eq1 = Equipment(
            ...
            brand="Yamato",
            installation_date=today - relativedelta(months=11, days=5), # installed ~11 mos ago
            lifespan_months=12,
        ...
        await db.commit()
        print("Successfully injected 3 mock equipments for testing!")
